...
function CheckAuthentication() {
    // go back to index to check the session
    $_GET['auth'] = true;
    require_once ('../../../../../../index.php');
	return isset($_SESSION['is_adm']) && $_SESSION['is_adm'];
}
...
/*
 ...
 Do not do this on a production server. Might be helpful when debugging why QFinder does not work as expected.
*/
error_reporting(E_ALL);
ini_set('display_errors', 1);

/*
To make it easy to configure QFinder, the $baseUrl and $baseDir can be used.
...
in QFinder. If empty, the default value (/userfiles/) is used.

Examples:
	$baseUrl = 'http://example.com/qfinder/files/';
	$baseUrl = '/userfiles/';
...
above $baseUrl URL. This is the path used by QFinder to handle the files in
the server. Full write permissions must be granted to this directory.

Examples:
	// You may point it to a directory directly:
	$baseDir = '/home/login/public_html/qfinder/files/';
	$baseDir = 'C:/SiteDir/QFinder/userfiles/';

	// Or you may let QFinder discover the path, based on $baseUrl.
	// WARNING: resolveUrl() *will not work* if $baseUrl does not start with a slash ("/"),
	// for example if $baseDir is set to  http://example.com/qfinder/files/
...
/*
RoleSessionVar : the session variable name that QFinder must use to retrieve
...
*/
$config['RoleSessionVar'] = 'QFinder_UserRole';
//session_start();
...
/*
ResourceType : defines the "resource types" handled in QFinder. A resource
...
When loading QFinder, the "type" querystring parameter can be used to display
...
If enabled, QFinder will disallow creating folders and uploading files whose names contain characters
...
/*
Folders to not display in QFinder, no matter their location.
...
/*
Files to not display in QFinder, no matter their location.
...
For more complex configuration options visit our Developer's Guide
  http://docs.cksource.com/QFinder_2.x/Developers_Guide/PHP
*/
