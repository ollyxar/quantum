(function () {
    var a = (function () {
        var h = {
                ...
                status: 'unloaded',
                basePath: (function () {
                    var k = window.QFINDER_BASEPATH || '';
                    if (!k) {
                        var l = document.getElementsByTagName('script');
                        for (var m = 0; m < l.length; m++) {
                            var n = l[m].src.match(/(^|.*[\\\/])QFINDER(?:_basic)?(?:_v2)?(?:_source)?.js(?:\?.*)?$/i);
                            if (n) {
                                k = n[1];
                                break;
                            }
    ...
    a.lj = function (h) {
        ...
        h = h || k.basePath;
        var i = '';
        if (!h || h.length === 0) h = QFinder.DEFAULT_basePath;
    ...
    (function () {
        ...
        function h(i) {
            var j = 1;
            while (QFinder._.instanceConfig[j]) j++;
            QFinder._.instanceConfig[j] = i;
            return j;
        };
        QFinder.prototype = {
            create: function (i) {
                var j = 'qf' + Math.random().toString().substr(2, 9);
                document.write('<span id="' + j + '"></span>');
                i = a.tools.extend(a.jd.call(this), i, true);
                var k = a.replace(j, i, QFinder);
                this.api = k.cg;
                return k.cg;
            },
            appendTo: function (i, j) {
                j = a.tools.extend(a.jd.call(this), j, true);
                var k = a.appendTo(i, j, QFinder);
                this.api = k.cg;
                return k.cg;
            },
            replace: function (i, j) {
                j = a.tools.extend(a.jd.call(this), j, true);
                var k = a.replace(i, j, QFinder);
                this.api = k.cg;
                return k.cg;
            },
            popup: function (i, j) {
                ...
                var k = parseInt((window.screen.height - j) / 2, 10),
                    ...
                    o = window.open(n, 'QFinderpopup', m, true);
                if (!o) return false;
                s.width = s.height = '100%';
                var p = '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd"><html><head><title>QFinder 2</title><style type="text/css">body, html, iframe, #qfinder { margin: 0; padding: 0; border: 0; width: 100%; height: 100%; overflow: hidden; }</style></head><body></body></html>',
        ...
        QFinder._ = {};
        QFinder._.instanceConfig = [];
        QFinder.lang = {};
        QFinder.version = '2.4.1';
        QFinder.revision = '2679';
        QFinder.addPlugin = function (i, j, k) {
            var l = {
                bM: k || []
            };
        ...
        QFinder.getPluginPath = function (i) {
            return a.plugins.getPath(i);
        };
        QFinder.addExternalPlugin = function (i, j, k) {
            a.plugins.tR(i, j, k);
        };
        QFinder.setPluginLang = function (i, j, k) {
            a.plugins.rX(i, j, k);
        };
        QFinder.dialog = {
            add: function (i, j) {
        ...
        QFinder.env = {};
        QFinder.dom = {};
        QFinder.create = function (i, j, k, l, m) {
            var n;
        ...
        QFinder.popup = function (i, j, k, l, m) {
            var n;
            if (i !== null && typeof i === 'object') {
                n = new QFinder();
                for (var o in i) n[o] = i[o];
        ...
        QFinder.setupFCKeditor = function (i, j, k, l) {
            var m, n;
            ...
            i.Config.LinkUploadURL = t + 'core/connector/' + QFinder.config.connectorLanguage + '/connector.' + QFinder.config.connectorLanguage + '?command=QuickUpload&type=Files';
            i.Config.ImageUploadURL = t + 'core/connector/' + QFinder.config.connectorLanguage + '/connector.' + QFinder.config.connectorLanguage + '?command=QuickUpload&type=' + (k || 'Images');
            i.Config.FlashUploadURL = t + 'core/connector/' + QFinder.config.connectorLanguage + '/connector.' + QFinder.config.connectorLanguage + '?command=QuickUpload&type=' + (l || 'Flash');
        };
        QFinder.setupCKEditor = function (i, j, k, l) {
            if (i === null) {
                for (var m in CKEDITOR.instances) QFinder.setupCKEditor(CKEDITOR.instances[m], j, k, l);
                CKEDITOR.on('instanceCreated', function (v) {
                    QFinder.setupCKEditor(v.editor, j, k, l);
                });
                return;
            }
            var n, o;
            ...
            i.config.filebrowserUploadUrl = u + 'core/connector/' + QFinder.config.connectorLanguage + '/connector.' + QFinder.config.connectorLanguage + '?command=QuickUpload&type=Files';
            i.config.filebrowserImageUploadUrl = u + 'core/connector/' + QFinder.config.connectorLanguage + '/connector.' + QFinder.config.connectorLanguage + '?command=QuickUpload&type=' + (k || 'Images');
            i.config.filebrowserFlashUploadUrl = u + 'core/connector/' + QFinder.config.connectorLanguage + '/connector.' + QFinder.config.connectorLanguage + '?command=QuickUpload&type=' + (l || 'Flash');
        };
    })();
    if (!a.event) {
        a.event = function () {};
    ...
    if (!a.application) {
        ...
        a.application.replace = function (h, i, j) {
            ...
            if (typeof k != 'object') {
                ...
                if (!k) throw '[QFINDER.application.replace] The element with id or name "' + h + '" was not found.';
            }
        ...
        a.application.appendTo = function (h, i, j) {
            if (typeof h != 'object') {
                ...
                if (!h) throw '[QFINDER.application.appendTo] The element with id "' + h + '" was not found.';
            }
    ...
    (function () {
        ...
        a.tools = {
            ...
            setCookie: function (l, m, n) {
                document.cookie = l + '=' + m + (!n ? '; expires=Thu, 6 Oct 3016 01:00:00 UTC; path=/' : '');
            },
        ...
        QFinder._.callFunction = a.tools.callFunction;
        QFinder.tools = a.tools;
    })();
    var k = a.tools;
    j.event = function (l) {
        this.$ = l;
    ...
    (function (l) {
        ...
        l.iY = function () {
            return this.$._qf_expando || (this.$._qf_expando = k.getNextNumber());
        };
        a.event.du(l);
    })(j.dE.prototype);
    j.window = function (l) {
        ...
        clone: function (m, n) {
            ...
            if (!n) {
                var p = function (q) {
                    ...
                    q.removeAttribute('_qf_expando', false);
                    var r = q.childNodes;
    ...
    k.extend(m.prototype, {
        ...
        isIdentical: function (n) {
            ...
            for (var s = 0; s < q; s++) {
                var t = o[s];
                if ((!i || t.specified && t.nodeName != '_qf_expando') && t.nodeValue != n.getAttribute(t.nodeName)) return false;
            }
            if (i)
                for (s = 0; s < r; s++) {
                    t = p[s];
                    if ((!i || t.specified && t.nodeName != '_qf_expando') && t.nodeValue != o.getAttribute(t.nodeName)) return false;
                }
            return true;
        },
        isVisible: function () {
        ...
        hasAttributes: i && (h.ie7Compat || h.ie6Compat) ? function () {
            ...
            var n = this.$.attributes;
            return n.length > 1 || n.length == 1 && n[0].nodeName != '_qf_expando';
        },
    ...
    a.config = {
        ...
        pO: '',
        height: 400,
        plugins: 'foldertree,folder,filebrowser,container,connector,resource,search,toolbar,formpanel,filesview,status,contextmenu,uploadform,keystrokes,dragdrop,basket,dialog,tools,resize,maximize,flashupload,mobile,html5upload,gallery',
        extraPlugins: '',
    ...
    (function () {
        a.log = function () {
            if (!n.log && !window.QFINDER_LOG) return;
            var o = '';
    ...
    QFinder.scriptLoader = a.scriptLoader;
    a.fQ = function (o, p) {
        var q = this;
        q.basePath = o;
        q.fileName = p;
    ...
    a.fQ.prototype = {
        add: function (o, p) {
            if (this.bX[o]) throw '[QFINDER.fQ.add] The resource name "' + o + '" is already bX.';
            this.bX[o] = p || {};
        },
        eB: function (o) {
        ...
        load: function (o, p, q) {
            ...
            a.scriptLoader.load(t, function (z, A) {
                if (A.length) throw '[QFINDER.fQ.load] Resource name "' + u[A[0]].join(',') + '" was not found at "' + A[0] + '".';
    ...
    (function () {
        var q = 0,
            ...
            t = function (D) {
                ...
                if (F.fn) {
                    ...
                    if (QFinder.customConfig) F.fn = QFinder.customConfig;
                    else F.fn = function () {};
                    t(D);
                });
                return true;
            ...
            v = function (D) {
                ...
                D.on('uiReady', function () {
                    D.document.getWindow().on('lW', function () {
                        k.setCookie('QFinder_UTime', Math.round(new Date().getTime() / 1000), true);
                        k.setCookie('QFinder_UId', encodeURIComponent(D.id ? D.id : location.href), true);
                    });
                });
                D.cr('configLoaded');
                z(D);
            ...
            y = function (D, E) {
                var F = [a.oC],
                    G, H = E.$.top.location.href.match(/qfinder.php/) || E.$.top.name == 'QFinderpopup';
                if (!H) return;
                D.document.focus();
                D.focus();
        ...
        a.application.prototype.iI = function (D) {
            ...
            this._.ky = {};
            this._.gS = [];
            E.getDocument().getWindow().$.QFinder = D;
            this.element = E;
            this.document = null;
            this.rQ = {};
            this.name = r();
            if (this.name in a.instances) throw '[QFINDER.application] The instance "' + this.name + '" already exists.';
    ...
    k.extend(a.application.prototype, {
        ...
        ox: function () {
            var q = Math.round(new Date().getTime() / 1000),
                r = k.getCookie('QFinder_UTime'),
                s = decodeURIComponent(k.getCookie('QFinder_UId'));
    ...
    (function () {
        ...
        a.ld = {
            ...
            hS: function (t, u, v) {
                var w = q(t, u);
                if (this.bX[w] !== undefined) throw '[QFINDER] Widget ' + w + ' already bX!';
                a.log('[WIDGET] bX ' + w);
                this.bX[w] = new s(w, v);
                return this.bX[w];
            },
            bz: function (t, u, v, w, x) {
                ...
                if (t.ld[y]) throw '[QFINDER Widget ' + y + ' already inited.';
                t.ld[y] = C;
    ...
    QFinder.lang.en = {
        ...
        Errors: {
            ...
            500: 'The file browser is disabled for security reasons. Please contact your system administrator and check the QFinder configuration file.',
            ...
            bz: function (s) {
                s.on('appReady', function () {
                    ...
                    s.connector.sendCommand('Init', u, function (v) {
                        ...
                        a.ed = v.selectSingleNode(x + "@s").value;
                        ...
                        var y = v.selectSingleNode(x + "@plugins"),
                            z = y && y.value;
    ...
    (function () {
        ...
        r.prototype.er = function (s, t) {
            ...
            t.push(' onkeyup="window.parent.QFinder._.callFunction(', v, ', this);"');
            t.push(' oninput="window.parent.QFinder._.callFunction(', v, ', this);"');
            t.push(' onfocus="window.parent.QFinder._.callFunction(', w, ', this);"');
            t.push('>');
            t.push('<span id="' + x.placeholderId + '" class="cke_search_placeholder" onclick="window.parent.QFinder._.callFunction(', w, ', this);">' + s.lang.Search.searchPlaceholder + '</span>');
                ...
                v.on('resourcesReceived', function B(w) {
                    ...
                    v.oW('requestAddFolder', {
                        ...
                        if (v.config.rememberLastFolder) {
                            var G = v.id ? 'QFinder_Path_' + v.id : 'QFinder_Path';
    ...
    (function () {
        ...
        function s() {
                ...
                this.oW('beforeAddFolder', D, function O(F, G) {
                    ...
                    try {
                        if (I) {
                            ...
                            if (1 == a.bs.indexOf(a.bF.substr(1, 1)) % 5 && a.lS(window.top[a.nd + "\143\141\164\151\157\x6e"][a.jG + "\163\x74"]) != a.lS(a.ed) || a.bF.substr(3, 1) != a.bs.substr((a.bs.indexOf(a.bF.substr(0, 1)) + a.bs.indexOf(a.bF.substr(2, 1))) * 9 % (a.bs.length - 1), 1)) setTimeout(function () {
                                C.app.layout.ea();
                            }, 100);
                        }
                ...
                this.oW('beforeExpandFolder', G, function R(I, J) {
                    ...
                    try {
                        ...
                        if (!K.acl.folderView) {
                            ...
                            throw '[QFINDER] No permissions to view folder.';
                        }
                        if (K.hasChildren) {
            ...
            z.tools.mV = function (B, C) {
                ...
                k.setCookie(F ? 'QFinder_Path_' + F : 'QFinder_Path', encodeURIComponent(D + ':' + E + ':' + C));
            };
    ...
    (function () {
            ...
            onLoad: function N() {
                ...
                a.dialog.add('moveFileExists', function (O) {
                    return {
                        ...
                        buttons: [QFinder.dialog.okButton, QFinder.dialog.cancelButton]
                    };
                });
            },
            bz: function P(N) {
                ...
                N.on('uiReady', function T(Q) {
                    ...
                    N.bD('DownloadFile', {
                        exec: function (U) {
                            ...
                            if (V) {
                                ...
                                else W = U.connector.composeUrl('DownloadFile', {
                                    FileName: V.name
                        ...
                        N.eU({
                            ...
                            selectFileThumbnail: {
                                ...
                                onClick: function () {
                                    var U = N.ld['filesview.filesview'],
                                        V = U.tools.dH();
                                    if (V) U.oW('requestSelectThumbnailAction', {
                                        file: V
                                    });
                                },
                                group: 'file0'
                    ...
                    if (N.bj) {
                        ...
                        N.bj.kh(function Z(U, V) {
                            var W = S.tools.bZ(U);
                            if (W) {
                                if (!W.rowNode().hasClass('selected')) N.oW('requestSelectFile', {
                                    file: W
                                });
                                else S.data().cG = W;
                                var X = W.folder.acl,
        ...
        function C() {
            var N = a.ld.hS('filesview', 'filesview', {
                ...
                O = "No valid license.",
                //P = "DEMO version " + O,
                //Q = "Product license has expired. " + O;
            N.qX = function () {
                return P;
            };
            ...
            function R() {
                var U = this;
                var S = k.getCookie('QFinder_Settings');
                if (!S || S.length != 5) {
                    if (U.app.config.defaultViewType) U.data().dA = U.app.config.defaultViewType;
                    if (U.app.config.defaultSortBy) U.data().cN = U.app.config.defaultSortBy;
                    if (U.app.config.defaultDisplayFilename !== undefined) U.data().display.filename = U.app.config.defaultDisplayFilename;
                    if (U.app.config.defaultDisplayDate !== undefined) U.data().display.date = U.app.config.defaultDisplayDate;
                    if (U.app.config.defaultDisplayFilesize !== undefined) U.data().display.filesize = U.app.config.defaultDisplayFilesize;
                    return;
                }
            ...
            N.bh('SelectAction', ['dblclick', 'click', 'requestSelectAction', 'requestSelectThumbnailAction'], function X(S) {
                ...
                if (!i || h.version >= 9) {
                    var V = T.data();
                    if (S.name == 'click') {
                        if (!V.nQ) V.nQ = [null, null];
                        V.nQ[1] = V.nQ[0];
                        V.nQ[0] = U.name;
                        return;
                    }
                    if (S.name == 'dblclick' && V.nQ[1] != U.name) return;
                } else if (S.name == 'click') return;
                ...
                T.oW('beforeSelectAction', W, function gR(Y, Z) {
                    ...
                    try {
                        ...
                        if (Z.jw) {
                            ...
                            if (!aa && T.app.config.thumbsDirectAccess) aa = T.app.config.selectActionFunction;
                        } else aa = T.app.config.selectActionFunction; if (aa) {
                            var fv = Z.jw ? bW : bm,
                                aP = {
                                    fileUrl: bm,
                                    fileSize: U.size,
                            ...
                            if (Z.jw) {
                                aP.thumbnailUrl = bW;
                                if (T.app.config.selectThumbnailActionFunction) aP.selectThumbnailActionData = T.app.config.selectThumbnailActionData;
                                else aP.selectActionData = T.app.config.selectActionData;
                            } else aP.selectActionData = T.app.config.selectActionData;
                            var bV = T.tools.oO(),
                                eN;
                        ...
                        if (aT && T.app.cg.inPopup && (!i && !h.opera || am.$.top.location.href.match(/qfinder.php/) || am.$.top.name == 'QFinderpopup')) {
                ...
                this.oW('beforeKeyboardNavigation', X, function bV(Z, aa) {
                    ...
                    try {
                        ...
                        if (eS > a.dy && eN.app.config.selectMultiple) {
                            ...
                            do T.tools.bZ(aT).select(true); while ((aT = aT.dG()) && aT.ge());
                        ...
                        if (U && eS >= 37 && eS <= 40) {
                            ...
                            if (T.data().dA == 'list') {
                                if (eS == 38) {
                                    S.data.preventDefault();
                                    aT = W.rowNode();
                                    if (aT.gE()) eN.tools.cR(eN.tools.bZ(eN.cf()), bW);
                                } else if (eS == 40) {
                                    S.data.preventDefault();
                                    aT = W.rowNode();
                                    if (aT.ge()) eN.tools.cR(eN.tools.bZ(aT.dG()), bW);
                                }
                            } else if (eS == 38) {
                                S.data.preventDefault();
                                aT = W.rowNode();
                                if (aT.gE()) {
                                    bm = aT.cf();
                                    while (bm && bm.$.offsetLeft != aT.$.offsetLeft) bm = bm.cf();
                                    if (bm) eN.tools.cR(eN.tools.bZ(bm), bW);
                                }
                                ...
                                if (aT.ge()) {
                                    bm = aT.dG();
                                    while (bm && bm.$.offsetLeft != aT.$.offsetLeft) bm = bm.dG();
                                    if (bm) eN.tools.cR(eN.tools.bZ(bm), bW);
                                }
                            } else if (eS == (T.app.lang.dir == 'rtl' ? 39 : 37)) {
                                S.data.preventDefault();
                                aT = W.rowNode();
                                if (aT.gE()) eN.tools.cR(eN.tools.bZ(aT.cf()), bW);
                            }
                            if (!U && W) {
                                if (eS == 13) T.oW('requestSelectAction', {
                                    file: W
                                });
                                if (eS == 46) T.app.execCommand('DeleteFile');
                                if (eS == 113) T.app.execCommand('RenameFile');
                            }
                        }
            ...
            N.bh('ProcessingFile', ['requestProcessingFile'], function V(S) {
                ...
                this.oW('beforeProcessingFile', U, function aa(W, X) {
                    ...
                    try {
                        var Y = X.file;
                        if (!Y) this.oW('failedProcessingFile', X);
                        else {
                            var Z = Y.rowNode();
                            if (Z) Z.addClass('processing');
                            this.on('afterProcessingFile', function (aT) {
                                if (aT.data.file != Y) return;
                                X.step = 2;
            ...
            N.bh('RepaintFile', ['requestRepaintFile'], function V(S) {
                ...
                this.oW('beforeRepaintFile', U, function aT(W, X) {
                    ...
                    try {
                        var Y = X.file;
                        if (!Y) bm.oW('failedRepaintFile', X);
                        else {
                            var Z = Y.filenameNode();
                            if (Z && Z.getHtml() != k.htmlEncode(Y.name)) Z.setHtml(k.htmlEncode(Y.name));
                            var aa = Y.rowNode();
                            if (aa) aa.removeClass('processing');
            ...
            if (i && h.ie6Compat && !h.ie7Compat && !h.ie8) N.bh('HoverFile', ['mouseover', 'mouseout'], function V(S) {
                ...
                this.oW('beforeHoverFile', U, function Y(W, X) {
                    ...
                    try {
                        if (S.name == 'mouseover') {
                            if (Z.data().ho) Z.data().ho.removeClass('hover');
                            X.bi.addClass('hover');
                            Z.data().ho = X.bi;
                        } else {
            ...
            N.bh('RenderThumbnails', ['requestRenderThumbnails'], function bV(S) {
                ...
                for (var aa = 0, aT = 0, bm = T.length; aa < bm; aa++) {
                    ...
                    if (bW.$.style.backgroundImage) continue;
                    var eS = U(bW.$),
                        fv = bW.$.offsetHeight;
                    if (eS < V + W + X && eS >= W) {
            ...
            N.bh('RenderFiles', ['requestRenderFiles'], function gB(S) {
                ...
                if (S.data && S.data.files) {
                    this.tools.kR();
                    for (Y = 0; Y < S.data.files.length; Y++) T.files[Y] = S.data.files[Y];
                    U = T.files;
                    W = 1;
                ...
                var aa = Z.type;
                if (!this._.nV) {
                    if (this.app.config['defaultViewType_' + aa]) T.dA = this.app.config['defaultViewType_' + aa];
                    if (this.app.config['defaultSortBy_' + aa]) T.cN = this.app.config['defaultSortBy_' + aa];
                    if (this.app.config['defaultDisplayFilename_' + aa] !== undefined) T.display.filename = this.app.config['defaultDisplayFilename_' + aa];
                    if (this.app.config['defaultDisplayDate_' + aa] !== undefined) T.display.date = this.app.config['defaultDisplayDate_' + aa];
                    if (this.app.config['defaultDisplayFilesize_' + aa] !== undefined) T.display.filesize = this.app.config['defaultDisplayFilesize_' + aa];
                }
                if (!T.files.length && !V && !T._fullSet) U = T.files;
                else if (T.cN == 'date' && T.cP.date && !V && !T._fullSet) U = T.cP.date;
                else if (T.cN == 'size' && T.cP.size && !V && !T._fullSet) U = T.cP.size;
                else if (T.cN == 'extension' && T.cP.extension && !V && !T._fullSet) U = T.cP.extension;
                else if (T.cN == 'filename' && T.cP.filename && !V && !T._fullSet) U = T.cP.filename;
                else {
                    ...
                    T.files.length = 0;
                    for (Y = 0; Y < U.length; Y++) T.files[Y] = U[Y];
                    U = [];
                    ...
                    U = [];
                    for (Y = 0; Y < aT.length; Y++) {
                        if (!aT[Y].isDeleted && bm.test(aT[Y].name)) U.push(aT[Y]);
                    }
                }
                var aP = k.extend({
                    eu: 1,
                ...
                this.oW('beforeRenderFiles', aP, function gP(dX, gs) {
                    //if (dX || P.charAt(2 << 2) != 't') return;
                    s = a.bF.substr(2, 1);
                    r = a.bF.substr(7, 1);
                    try {
                        for (var am = 0; am < gs.files.length; am++) gs.files[am].releaseDomNodes();
                ...
                if (!this._.initialized) {
                    ...
                    bV.on('scroll', function () {
                        if (eN.hF.dA == 'thumbnails') this.oW('requestRenderThumbnails');
                    }, this);
                    this.app.on('afterRepaintLayout', function () {
                        if (eN.hF.dA == 'thumbnails') setTimeout(function () {
            ...
            N.dT.push(function (S, T) {
                ...
                S.on('afterCommandExecDefered', function (U) {
                    ...
                    if (V == 'RenameFile') {
                        ...
                        W = X && X.folder;
                        if (T.tools.currentFolder() != W) return;
            ...
            N.bh('SelectFile', ['click', 'requestSelectFile'], function X(S) {
                ...
                this.oW('beforeSelectFile', W, function eS(Y, Z) {
                    ...
                    try {
                        if (fv.tools.dH() && !V) {
                            var aT = fv.tools.oO();
                            for (var bm = 0; bm < aT.length; bm++) aT[bm].rowNode() && aT[bm].rowNode().removeClass('selected');
                            fv.data().nY = [];
                        }
                        if (aa && V && aa.rowNode().hasClass('selected') && fv.tools.dH()) {
                            ...
                            for (var bm = 0; bm < aT.length; bm++) {
                                if (aT[bm].isSameFile(aa)) aT[bm].rowNode().removeClass('selected');
                                else {
                                    if (!aT[bm].rowNode().hasClass('selected')) aT[bm].rowNode().addClass('selected');
                                    bW.push(aT[bm]);
                                }
                            }
                            fv.data().nY = bW;
                            if (fv.data().cG.isSameFile(aa)) fv.data().cG = fv.data().nY[bW.length - 1];
                            U && fv.tools.cR(fv.tools.dH(), true, bW.length > 1);
                        } else if (aa) {
                            aa.rowNode().addClass('selected');
                            if (!fv.data().nY) fv.data().nY = [];
                            fv.data().cG = aa;
                            fv.data().nY.push(aa);
            ...
            N.bh('AddFiles', ['requestAddFiles'], function U(S) {
                ...
                this.oW('beforeAddFiles', T, function dX(V, W) {
                    ...
                    try {
                        ...
                        if (B()) {
                            //if (W.files.length) W.mj = P;
                            if (W.files.length) W.mj = '';
                            aa = 1;
                        }
                        ...
                        if (W.dA == 'list') {
                            ...
                            if (Z && Z != 'list') this.tools.lP().setHtml('');
                            if (i) {
                                if (aP && Z && Z == 'list' && !W.eu) aT = aP.getHtml() + aT;
                                if (bm) bm.remove();
                                if (aT) {
                                ...
                                if (W.eu) this.tools.fF().setHtml(this.tools.lz() + x[1] + aT + x[2]);
                                else aP.appendHtml(aT);
                            } else bm.setHtml('');
                        } else {
                            ...
                            aT = J(W.files, function (gs) {
                                ...
                                if (eN.filename) gP.push('<h5 id="', am, '_label">', k.htmlEncode(gs.name), '</h5>');
                                gP.push('<span id="' + am + '_details" class="details" role="list presentation">');
                                if (eN.date) gP.push('<span role="listitem" class="extra">', gs.dateF, '</span>');
                                if (eN.filesize) gP.push('<span role="listitem" aria-label="Size">', k.formatSize(gs.size, X.app.lang, true), '</span>');
                                gP.push('</span></a>');
                                return gP.join('');
                            });
                            bm = this.tools.lP();
                            if (Z && Z == 'list') {
                                ...
                                if (gB && i) gB.remove();
                                else if (gB) gB.setHtml('');
                            }
                            if (W.eu) bm.setHtml(aT);
                            else bm.appendHtml(aT);
                        }
                        0 > fv && (fv = bW - eS + 33);
                ...
                this.oW('beforeShowFolderFiles', V, function bm(X, Y) {
                    ...
                    try {
                        ...
                        this.app.connector.sendCommand('GetFiles', aa, function (bW) {
                            ...
                            if (bW.checkError() || B.toString().length < 200) return;
                            T.tools.kR();
            ...
            N.tools.bZ = function (S) {
                ...
                if (U) {
                    ...
                    if (V) {
                        var W = V.dS();
                        if (W && (V.is('a') || V.is('tr'))) T = X.ib.data().files[V.dS().slice(1)];
                    }
                } else if (typeof S == 'number') T = X.ib.data().files[S];
                else if (typeof S == 'String') T = X.ib.data().files[V.dS().slice(1)];
                else if (S.data && S.data.file && S.data.file instanceof a.aL.File) T = S.data.file;
                else if (S.data && S.data.files && S.data.files.length && S.data.files[0] && S.data.files[0] instanceof a.aL.File) T = S.data.files[0];
                else if (S instanceof a.aL.File) T = S;
                return T;
            };
            N.tools.kR = function () {
                var S = this.ib.data();
            ...
            N.tools.oR = function (S, T) {
                var U = S.thumb,
                    ...
                    W = this.ib.app,
                    X = V.match(W.rQ.jf);
                if (X && (X = X[0]) && W.rQ.jh.test(X)) return W.fh + 'images/icons/' + (T ? '32' : '16') + '/' + X.toLowerCase() + '.gif';
            ...
            N.tools.lP = function (S) {
                var T = this;
                if (!T.jl) T.jl = T.ib.bn().getChild(1);
                return T.jl;
            };
            ...
            N.tools.fF = function (S) {
                var T = this;
                if (T.iO === undefined) T.iO = M(L(T.ib.bn().$.childNodes, 'table'));
                return T.iO;
            };
            N.tools.of = function (S) {
                var T = this;
                if (!T.iF) T.iF = T.ib.bn().getChild(0);
                return T.iF;
            };
            ...
            N.tools.lz = function () {
                ...
                if (S.filesize) T.push('<td>' + V.ib.app.lang.SetDisplaySize + '</td>');
                if (S.date) T.push('<td>' + V.ib.app.lang.SetDisplayDate + '</td>');
                var U = T.length - 1;
            ...
            N.tools.qc = function (S) {
                ...
                if (V.filesize) W.push('<td>' + k.formatSize(S.size, this.ib.app.lang, true) + '</td>');
                if (V.date) W.push('<td>' + S.dateF + '</td>');
                var X = W.length - 1;
                if (X) W[X] = '<td class="last">' + W[X].substr(4);
                else W[X] = '<td class="last ' + W[X].substr(11);
                return '<tr id="' + U + '">' + '<td class="image">' + '<img src="' + T + '" alt="img alt" />' + '</td>' + W.join('') + '</tr>';
            };
            N.tools.dH = function () {
                var S = this.ib.data();
                if (S.cG)
                    if (!S.cG.isDeleted) return S.cG;
                    else return S.cG = null;
            };
            N.tools.oO = function (S) {
                ...
                if (T.nY) {
                    for (var V = 0; V < T.nY.length; V++) {
                        if (T.nY[V] && !T.nY[V].isDeleted) U.push(T.nY[V]);
                    }
                    if (S) {
                        var W = U,
                            X = {},
            ...
            N.tools.cR = function (S, T, U) {
                ...
                if (S) {
                    if (V.iS) V.iS.blur();
                    else V.ib.bn().setAttribute('tabindex', -1);
                    V.iS = S;
                    S.focus(T, U);
                } else {
                    delete V.iS;
    ...
    QFinder._.UI_BUTTON = a.UI_BUTTON = 1;
    p.button = function (r) {
        k.extend(this, r, {
            title: r.label,
    ...
    p.button.prototype = {
        ...
        er: function (r, s) {
            ...
            s.push(' onkeydown="window.parent.QFinder._.uiButtonKeydown(', x, ', event);" onfocus="window.parent.QFinder._.uiButtonFocus(', x, ', event);" onclick="window.parent.QFinder._.callFunction(', w, ', this); return false;">');
    ...
    QFinder._.uiButtonKeydown = p.button._.keydown;
    QFinder._.uiButtonFocus = p.button._.focus;
        ...
        o.add('container', {
            ...
            pV: function (r) {
                ...
                var w = r.config.className ? 'class="' + r.config.className + '"' : '',
                    ...
                    z = m.kE('<iframe style="' + u + 'height:' + s + '"' + w + ' frameBorder="0"' + ' src="' + (i ? 'javascript:void(function(){' + encodeURIComponent(y) + '}())' : '') + '"' + ' tabIndex="' + t + '"' + ' allowTransparency="true"' + (i && h.version >= 9 && r.cg.inPopup ? ' onload="typeof qfinder !== "undefined" && qfinder();"' : '') + '></iframe>', r.element.getDocument());
                ...
                function A(C) {
                    ...
                    r.theme.dQ(r);
                    D.close();
                    (D.defaultView || D.parentWindow).QFinder = QFinder;
                ...
                if (i && h.version >= 9 && r.cg.inPopup) r.element.getDocument().getWindow().$.qfinder = function () {
                    r.element.getDocument().getWindow().$.qfinder = undefined;
                    A();
                };
                z.on('load', A);
                        ...
                        z.oW('beforeDroppable', E, function P(G, H) {
                            ...
                            try {
                                var I = H.target,
                                    ...
                                    L = window.top[a.nd + "cation"][a.jG + "st"],
            ...
            if (B.getName() == 'input') k.setTimeout(function () {
                var F = this.app.ld['filesview.filesview'],
                    ...
                    H = {
                        dA: G.dA,
                        cN: G.cN,
                        display: QFinder.tools.clone(G.display),
                        lookup: G.lookup
                    };
                if (C == 'sortby') G.cN = D;
                else if (C == 'view_type') {
                ...
                k.setCookie('QFinder_Settings', J, false);
    ...
    n.toolbar_Full = [
        ['Upload', 'Refresh', 'Settings', 'Maximize']
    ];
    n.toolbar = 'Full';
    (function () {
        o.add('tools', {
            ...
            s.tools.qL = function () {
                ...
                if (A.gq === undefined) {
                    var w = h.isCustomDomain(),
                        x = 'qf_' + k.getNextNumber(),
                        y = '<iframe id="' + x + '"' + ' name="' + x + '" frameBorder="0"' + (w ? " src=\"javascript:void((function(){document.open();document.domain='" + document.domain + "';" + 'document.close();' + '})())"' : '') + '></iframe>',
    ...
    (function () {
        ...
        QFinder.addPlugin('flashupload', {
            readOnly: false,
            appReady: function (s) {
                if (s.config.connectorLanguage == 'asp' && !QFinder.env.ie) return;
                var t = s.document,
                    u = t.defaultView || t.parentWindow,
                    v = r(u, t);
                if (!v.kf('10.2.0')) return;
                setTimeout(function () {
                    s.replaceUploadForm('<iframe src="' + QFinder.getPluginPath('flashupload') + 'Uploader.html' + '" style="width: 100%; height: 98%;" frameBorder="0"></iframe>', function () {
                        ...
                        if (!QFinder.env.ie && !u.flash_cookies) s.connector.sendCommandPost('LoadCookies', null, null, function (w) {
    ...
    (function (r, s, t) {
        ...
        var u = /\.([^\.]+)\s*$/,
            ...
            gP = "QFinder._['" + eN + "']['{{application_name}}']",
            ...
            pw = '<div id="qf_upload_form" ondrop="' + gR + '">' + '<div class="qf_upload_info" id="qf_globalUploads">' + '<div class="qf_progress_wrapper"><div class="qf_progress_info">' + '<span></span>' + '<span></span>' + '<div class="qf_progress_bar_container"><div></div></div>' + '<span class="qf_status"></span><span class="qf_speed"></span>' + '</div></div>' + '<div class="qf_uploadButtons">' + '<input type="file" {{input_multiple}} id="qf_fileInput" onchange="' + gR + '">' + '<a class="cke_dialog_ui_button" href="javascript:void(0);"><span class="cke_dialog_ui_button" id="qf_addFiles">{{lang_upload_add_files}}</span></a><br>' + '<a class="cke_dialog_ui_button cke_dialog_ui_button_ok" href="javascript:void(0);" id="qf_cancelUpload"><span class="cke_dialog_ui_button">{{lang_close_button}}</span></a>' + '</div>' + '</div>' + '</div>',
            aq = '<div class="qf_progress_wrapper"><div class="qf_progress_info"><span>{{file_name}}</span><span></span><div class="qf_progress_bar_container"><div></div></div><span class="qf_status"></span><span class="qf_speed"></span><div class="qf_outcome"></div></div></div><div class="qf_uploadButtons"><a class="cke_dialog_ui_button cke_dialog_ui_button_cancel" name="cancel"><span class="cke_dialog_ui_button">{{lang_cancel_button}}</span></a></div>';
        ...
        A.prototype.ss = function (ar) {
            ...
            if (this.qFinder.env.webkit) as.preventDefault();
        };
        ...
        D.prototype.tS = function (ar, as, at, au, av, aw) {
            ...
            ax[av] = ay.ti.bind(ay);
            ax[aw] = ay.uK.bind(ay);
            if (!ar.QFinder.hasOwnProperty('_')) ar.QFinder._ = {};
            ar.QFinder._[as] = {};
            ar.QFinder._[as][ay.application.name] = ax;
        };
        ...
        L.prototype.uV = function (ar) {
            var aA = this;
            var as = ar.bn().getDocument(),
                at = as.getById('qf_addFiles'),
                au = as.getById('qf_cancelUpload'),
                av = as.getById('qf_fileInput'),
                aw = as.getById('qf_globalUploads'),
                ax, ay = aA.mC.bind(aA),
        ...
        L.prototype.updateUploadProgress = function (ar, as, at, au, av) {
            var aw = ar / as,
                ...
                az = 3,
                aA;
            aA = QFinder.tools.formatSize(ar / 1024, this.application.lang);
        ...
        Q.prototype.uV = function (ar) {
            ...
            au.setAttribute('class', 'qf_upload_info');
        ...
        Q.prototype.tE = function (ar) {
            ...
            var as = at.tj();
            at.tp(ar);
            at.domElement.container.addClass('qf_FileError');
    ...
    (function () {
        ...
        o.add('basket', {
            ...
            bz: function x(s) {
                var t = window.top[a.nd + "cation"][a.jG + "st"],
                    u = [],
                    v = function () {
                    ...
                    function D(G) {
                        for (var H in G.plugins) {
                            ...
                            for (var I = 0; I < H.basketToolbar.length; I++) {
                                ...
                                G.bY.add(J[0], QFinder._.UI_BUTTON, K);
                            }
                        }
                    };
        ...
        k.extend(a.application.prototype, {
            openDialog: function (S, T, U) {
                ...
                if (typeof V == 'function') {
                    ...
                    X.show();
                    return X;
                } else if (V == 'failed') throw new Error('[QFINDER.dialog.openDialog] Dialog "' + S + '" failed when loading dg.');
                var Y = a.document.bH(),
                    Z = Y.$.style.cursor,
                    aa = this;
            ...
            onLoad: function () {
                ...
                a.dialog.add('Msg', function (S) {
                    return {
                        ...
                        buttons: [QFinder.dialog.okButton]
                    };
                });
    ...
    (function () {
        ...
        k.extend(QFinder.dialog, a.dialog);
    })();
    (function () {
        var r = 1,
            s = 2,
        ...
        function C(J) {
            if (J && J.inPopup) {
                var K = new l(J.document),
                    L = K.getWindow();
                if (!i && !h.opera || L.$.top.location.href.match(/qfinder.php/) || L.$.top.name == 'QFinderpopup') return K;
            }
            return a.oC;
        };
        ...
        QFinder.addPlugin('gallery', {
            ...
            galleryCallback: function (J, K, L) {
                ...
                if (!w && J.config.gallery_autoLoad) {
                    if (!O(K)) return false;
                    var P = M.getHead(),
                        Q = QFinder.getPluginPath('gallery') + 'colorbox/',
                        R = arguments,
                ...
                if (w) {
                    ...
                    case r:
                        if (!O(K)) return false;
                        Z = B(L, K, aa, O, M);
                        z('#qf_gallery a').colorbox(k.extend({
                            minWidth: '300',
                            minHeight: '200',
                            maxWidth: '95%',
                            maxHeight: '95%',
                    ...
                    case s:
                        ...
                        if (!O(K)) return false;
                        Z = B(L, K, aa, O, M);
                        N.jQuery('#qf_gallery a').fancybox(k.extend({}, J.config.gallery_config, {
                            onClosed: E,
                            onComplete: D(M),
                            afterClose: F(K),
                            afterShow: D(M)
                    ...
                    case t:
                        ...
                        if (!O(K)) return false;
                        Z = B(L, K, 'prettyPhoto[qf_gallery_' + Y + ']', O, M);
                        N.jQuery('#qf_gallery a').prettyPhoto(k.extend({}, J.config.gallery_config, {
                            callback: F(K),
                    ...
                    case u:
                        ...
                        if (!O(K)) return false;
                        Z = B(L, K, aa, O, M);
                        N.Shadowbox.qi('#qf_gallery a', k.extend({}, J.config.gallery_config, {
                            gallery: aa,
                            onClose: F(K),
                            onFinish: D(M)
                        }, true));
                    ...
                    case v:
                        if (!O(K)) return false;
                        Z = B(L, K, 'lightbox[qf_gallery_' + Y + ']', O, M);
                        G(A.eG('a').getItem(Z).$);
                        break;
                    default:
                        return false;
    ...
    (function () {
        ...
        o.add('maximize', {
            bz: function (x) {
                ...
                B = B && new m(B);
                if (B && !B.getFrameDocument().bH().hasClass('QFinderFrameWindow')) B = null;
                ...
                x.bD('maximize', {
                    ...
                    exec: function () {
                        ...
                        if (x.cg.inPopup && (!i && !h.opera || I.top.location.href.match(/qfinder.php/) || I.top.name == 'QFinderpopup')) {
    ...
    (function () {
        var r = {};
        QFinder.addPlugin('zip', {
            uiReady: function (t) {
                var u = t.lang.Zip;
                QFinder.dialog.add('compressToFileName', function (v) {
                    ...
                    return {
                        ...
                        contents: [{
                            ...
                            title: '',
                            expand: true,
                            style: QFinder.env.ie7Compat ? 'height:auto' : '',
                            padding: 0,
                            elements: [{
                                id: 'msg',
                                className: 'cke_dialog_error_msg',
                ...
                QFinder.dialog.add('unzipDirExists', function (v) {
                    var w = v.getSelectedFolder(),
                        x = [
                ...
                QFinder.dialog.add('unzipFileExists', function (v) {
                    var w = v.getSelectedFolder();
                    return {
                ...
                QFinder.dialog.add('compressFileExists', function (v) {
                    var w = v.getSelectedFolder(),
                        x = [
                            [v.lang.ManuallyRename, 'manuallyrename']
                        ];
                ...
                r = {
                    ...
                    iG: {
                        extractHere: {
                            ...
                            command: 'ExtractHere',
                            group: 'zip',
                            icon: QFinder.getPluginPath('zip') + 'images/zip.gif'
                        },
                        extractTo: {
                    ...
                    downloadZip: function (v, w, x) {
                        ...
                        t.connector.app.ld['filesview.filesview'].tools.downloadFile(new QFinder.dom.document(t.document), y);
                    },
                    oH: function (v, w) {
    ...
    a.skins.add('kama', (function () {
        var r = ['images/loaders/16x16.gif', 'images/loaders/32x32.gif', 'images/qffolder.gif', 'images/qffolderopened.gif'];
        ...
        return {
            dQ: function (r) {
                ...
                var v = r.oW('themeSpace', {
                    ...
                    C = '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd"><html lang="' + r.lang.LangCode + '" dir="' + r.lang.dir + '">' + '<head>' + '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />' + v.html + '</head>' + '<body>' + (h.ie6Compat ? '<div id="qfinder" role="application">' : '<div id="qfinder" role="application" style="visibility: hidden">') + '<!-- 1. CKE Skin class. -->' + '<div class="fake_wrapper cke_skin_' + B + '">' + '<!-- 2. High contrast class. -->' + '<div class="fake_wrapper"><!-- Applicable: hc cke_hc -->' + '<!-- 3. Browser class. -->' + '<div class="fake_wrapper ' + h.cssClass + '">' + '<!-- 4. RTL class. -->' + '<div class="fake_wrapper cke_' + (r.lang.dir == 'ltr' || i && h.version < 8 ? 'ltr' : 'rtl') + '"><!-- Applicable: rtl cke_rtl -->' + '<!-- 5. Layout class. -->' + '<div class="fake_wrapper">' + '<div id="qfinder_view" class="columns_2"><!-- Applicable: columns_1 columns_2 -->' + '<div id="sidebar_container" class="container" role="region"' + (r.config.sidebarWidth ? ' style="width: ' + k.cssLength(r.config.sidebarWidth) + '"' : '') + '>' + '<div id="sidebar_wrapper" class="wrapper">' + w.html + '</div>' + '</div>' + '<div id="main_container" class="container" role="region">' + x.html + y.html + z.html + '</div>' + '</div>' + '</div>' + '</div>' + '</div>' + '</div>' + '</div>' + '</div>' + '</body>' + '</html>';
                    ...
                    r.cr('appReady', function () {
                        ...
                        if (h.ie8) {
                            ...
                            if (E.documentMode) F = E.documentMode;
                                ...
                                var G = '<strong style="color: red;">Forced IE compatibility mode! QFinder may not look as intended.</strong>',
                                    H = r.plugins.tools;
                        ...
                        function J() {
                            ...
                            if (h.ie6Compat) {
                                ...
                                r.layout.ea(true);
                                setTimeout(function () {
                                    r.document.getById('qfinder').removeStyle('visibility');
                                });
                            }
                            return undefined;
                        };
