...
class QFinder_Connector_CommandHandler_Unzip extends QFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
    protected $filePath;
    ...
    protected function buildXml()
    {
        ...
        $this->unzippedNodes = new QFinder_Connector_Utils_XmlNode("UnzippedFiles");

        // list of files which could not be unzipped
        $this->skippedFilesNode = new QFinder_Connector_Utils_XmlNode("Errors");
        $this->errorCode = QFINDER_CONNECTOR_ERROR_NONE;
    }
    ...
    protected function checkOneFile($filePathInfo, $originalFileName)
    {
        ...
        if (copy('zip://' . $this->filePath . '#' . $originalFileName, $dir . $sFileName)) {
            ...
            if ($htmlExtensions
                ...
                && QFinder_Connector_Utils_FileSystem::detectHtml($dir . $sFileName) === true
            ) {
            ...
            if ($secureImageUploads
                && ($isImageValid = QFinder_Connector_Utils_FileSystem::isImageValid($dir . $sFileName, $sExtension)) === false
            ) {
                $this->errorCode = QFINDER_CONNECTOR_ERROR_UPLOADED_INVALID;
                $this->appendErrorNode($this->skippedFilesNode, $this->errorCode, $originalFileName);
                return false;
    ...
    {
        $oUnzippedNode = new QFinder_Connector_Utils_XmlNode("File");
        $oUnzippedNode->addAttribute("name", QFinder_Connector_Utils_FileSystem::convertToConnectorEncoding($name));
...
} // end of QFinder_Connector_CommandHandler_Unzip class

class QFinder_Connector_CommandHandler_UnzipHere extends QFinder_Connector_CommandHandler_Unzip
{
    /**
     * Handle request and build XML
     */
    protected function buildXml()
    {
                    ...
                    if (!is_dir($extractPath . $sFileName)) {
                        ...
                        if (!empty($checkedFiles[$fileName]['options'])) {
                            if ($checkedFiles[$fileName]['options'] == 'autorename') {
                                $sFileName = QFinder_Connector_Utils_FileSystem::autoRename($extractPath, $sFileName);
                                ...
                                if (!QFinder_Connector_Utils_FileSystem::unlink($extractPath . $sFileName)) {
                                    $this->errorCode = QFINDER_CONNECTOR_ERROR_ACCESS_DENIED;
                                    $this->appendErrorNode($this->skippedFilesNode, $this->errorCode, $fileName, $fileType);
                                } else {
                                    $this->extractTo($extractPath, $extractClientPath, $filePathInfo, $sFileName, $fileName);
                            ...
                            $this->errorCode = QFINDER_CONNECTOR_ERROR_ALREADY_EXIST;
                            $this->appendErrorNode($this->skippedFilesNode, $this->errorCode, $fileName, $fileType);
                        }
                    }
        ...
        if ($this->errorCode != QFINDER_CONNECTOR_ERROR_NONE) {
            $this->_connectorNode->addChild($this->skippedFilesNode);
            $this->_errorHandler->throwError(QFINDER_CONNECTOR_ERROR_ZIP_FAILED);
        }
    }
...
} // end of QFinder_Connector_CommandHandler_UnzipHere class

class QFinder_Connector_CommandHandler_UnzipTo extends QFinder_Connector_CommandHandler_Unzip
{
    /**
     * Handle request and build XML
     */
    protected function buildXml()
    {
        ...
        $extractDir = QFinder_Connector_Utils_FileSystem::convertToFilesystemEncoding($extractDir);
        if (preg_match(QFINDER_REGEX_INVALID_PATH, $extractDir)) {
            $this->_errorHandler->throwError(QFINDER_CONNECTOR_ERROR_INVALID_REQUEST);
        }
        $extractPath = QFinder_Connector_Utils_FileSystem::combinePaths($this->_currentFolder->getServerPath(), $extractDir . '/');
        $extractClientPath = QFinder_Connector_Utils_FileSystem::combinePaths($this->_currentFolder->getClientPath(), $extractDir);
        // acl for upload dir
        $_aclConfig = $this->_config->getAccessControlConfig();
        $aclMask = $_aclConfig->getComputedMask($this->_currentFolder->getResourceTypeName(), $extractDir);

        if (!(($aclMask & QFINDER_CONNECTOR_ACL_FOLDER_CREATE) == QFINDER_CONNECTOR_ACL_FOLDER_CREATE)) {
            $this->_errorHandler->throwError(QFINDER_CONNECTOR_ERROR_UNAUTHORIZED);
        }
        if (empty($_POST['force']) && file_exists($extractPath) && is_dir($extractPath) && !QFinder_Connector_Utils_FileSystem::isEmptyDir($extractPath)) {
            $dirExists = new QFinder_Connector_Utils_XmlNode("FolderExists");
            $oErrorNode = new QFinder_Connector_Utils_XmlNode("Folder");
            ...
            if (!(($aclMask & QFINDER_CONNECTOR_ACL_FILE_UPLOAD | QFINDER_CONNECTOR_ACL_FILE_DELETE) == QFINDER_CONNECTOR_ACL_FILE_UPLOAD | QFINDER_CONNECTOR_ACL_FILE_DELETE)) {
                $this->_errorHandler->throwError(QFINDER_CONNECTOR_ERROR_UNAUTHORIZED);
            }
            if ($extractDir && file_exists($extractPath) && is_dir($extractPath)) {
                if (!(($aclMask & QFINDER_CONNECTOR_ACL_FOLDER_CREATE | QFINDER_CONNECTOR_ACL_FOLDER_DELETE) == QFINDER_CONNECTOR_ACL_FOLDER_CREATE | QFINDER_CONNECTOR_ACL_FOLDER_DELETE)) {
                    $this->_errorHandler->throwError(QFINDER_CONNECTOR_ERROR_UNAUTHORIZED);
                }
                if (!QFinder_Connector_Utils_FileSystem::unlink($extractPath)) {
                    $this->_errorHandler->throwError(QFINDER_CONNECTOR_ERROR_ACCESS_DENIED);
                }
            }
...
} // end of QFinder_Connector_CommandHandler_UnzipTo class


class QFinder_Connector_CommandHandler_CreateZip extends QFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
    protected $_config;
...
} // end of QFinder_Connector_CommandHandler_DownloadZip class

class QFinder_Connector_CommandHandler_DownloadZip extends QFinder_Connector_CommandHandler_CreateZip
{
    /**
...
} // end of QFinder_Connector_CommandHandler_DownloadZip

if (extension_loaded('zip')) {
    $CommandHandler_UnzipHere = new QFinder_Connector_CommandHandler_UnzipHere();
    $CommandHandler_UnzipTo = new QFinder_Connector_CommandHandler_UnzipTo();
    $CommandHandler_CreateZip = new QFinder_Connector_CommandHandler_CreateZip();
    $CommandHandler_DownloadZip = new QFinder_Connector_CommandHandler_DownloadZip();
