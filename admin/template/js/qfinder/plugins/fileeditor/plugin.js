QFinder.addPlugin('fileeditor', function (api) {
    var regexExt = /^(.*)\.([^\.]+)$/,
    ...
    var codemirrorPath = QFinder.getPluginPath('fileeditor') + 'codemirror/';
    ...
    QFinder.dialog.add('fileEditor', function (api) {
        var height, width;
        ...
        return {
            ...
            onShow: function () {
                ...
                doc.getById('fileArea').setHtml('<div class="qfinder_loader_32" style="margin: 100px auto 0 auto;text-align:center;"><p style="height:' + cssHeight + 'px;width:' + cssWidth + 'px;">' + api.lang.Fileeditor.loadingFile + '</p></div>');
                ...
                // If QFinder is runninng under a different domain than baseUrl, then the following call will fail:
                // QFinder.ajax.load( file.getUrl() + '?t=' + (new Date().getTime()), function( data )...
                ...
                QFinder.ajax.load(url, function (data) {
                    ...
                    if (QFinder.env.opera)
                        doc.getById('fileContent').setHtml(QFinder.tools.htmlEncode(data));
                    else
                        doc.getById('fileContent').setText(data);

                    codemirror = null;
                    if (enableCodeMirror) {
                        QFinder.scriptLoader.load(codemirrorPath + 'lib/codemirror.js', function () {
                            QFinder.scriptLoader.load(codeMirrorParsers[ file.ext ], function () {
                    ...
                    elements: [
                        {
                            ...
                            html: '' +
                                ...
                                ( QFinder.env.gecko && QFinder.env.version >= 120000 ? '#fileArea .CodeMirror-scroll > div > div {position:absolute !important}' : '' ) +
                                '</style>' +
                                '<div id="fileArea"></div>'
                        }
                    ]
            ...
            buttons: [ saveButton, QFinder.dialog.cancelButton ]
        };
    });
    ...
    api.addFileContextMenuOption({ label: api.lang.Fileeditor.contextMenuName, command: 'fileEditor' }, function (api, file) {
        ...
        function (file) {
            ...
            if (typeof ( QFinder.config.fileeditorMaxSize ) != 'undefined')
                maxSize = QFinder.config.fileeditorMaxSize;
