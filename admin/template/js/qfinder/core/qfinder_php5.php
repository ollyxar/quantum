...
define('QFINDER_DEFAULT_BASEPATH', '/admin/template/js/qfinder/');
...
class QFinder
{
    ...
    // PHP 5 Constructor
    function __construct($basePath = QFINDER_DEFAULT_BASEPATH, $width = '100%', $height = 400, $selectFunction = null)
    {
    ...
    // Renders QFinder in the current page.
    public function Create()
    {
        echo $this->CreateHtml();
    }

    // Gets the HTML needed to create a QFinder instance.
    ...
    // Static "Create".
    public static function CreateStatic($basePath = QFINDER_DEFAULT_BASEPATH, $width = '100%', $height = 400, $selectFunction = null)
    {
    ...
    public static function SetupFCKeditor(&$editorObj, $basePath = QFINDER_DEFAULT_BASEPATH, $imageType = null, $flashType = null)
    {
        if (empty($basePath))
            $basePath = QFINDER_DEFAULT_BASEPATH;

        $qfinder = new QFinder($basePath);
        $qfinder->SetupFCKeditorObject($editorObj, $imageType, $flashType);
    }

    // Non-static method of attaching QFinder to FCKeditor
    ...
    // Non-static method of attaching QFinder to CKEditor
