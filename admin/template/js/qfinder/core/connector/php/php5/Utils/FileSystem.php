...
class QFinder_Connector_Utils_FileSystem
{
    /**
     * @param string $path
     * @return string
    ...
    public static function checkFileName($fileName)
    {
        ...
        if (preg_match(QFINDER_REGEX_INVALID_FILE, $fileName)) {
            return false;
        }
    ...
    public static function checkFolderName($folderName)
    {
        ...
        return QFinder_Connector_Utils_FileSystem::checkFileName($folderName);
    }
    ...
    public static function checkFolderPath($path){
      ...
      $path = explode('/',trim($path,'/'));
        foreach ( $path as $dir ){
          if ( !empty($dir) && !QFinder_Connector_Utils_FileSystem::checkFolderName($dir) ){
            return false;
          }
        }
    ...
    public static function unlink($path)
    {
        ...
        if ($files) {
            foreach($files as $filename)
            {
                ...
                QFinder_Connector_Utils_FileSystem::unlink($file);
            }
        }
  ...
  'Û' => 'U', 'Þ' => 'Th', 'Ð' => 'Dh', 'Æ' => 'Ae', 'Ĕ' => 'E',
    ...
    */
    ...
    {
        $serverPath = QFinder_Connector_Utils_FileSystem::combinePaths($_resourceType->getDirectory(), $clientPath);
    ...
    public static function sendFile( $filePath ){
      ...
      if ( $config->getXSendfile() ){
        QFinder_Connector_Utils_FileSystem::sendWithXSendfile($filePath);
      } else {
