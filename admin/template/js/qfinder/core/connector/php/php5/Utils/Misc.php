...
class QFinder_Connector_Utils_Misc
{
    public static function getErrorMessage($number, $arg = "") {
        $langCode = 'en';
        if (!empty($_GET['langCode']) && preg_match("/^[a-z\-]+$/", $_GET['langCode'])) {
            if (file_exists(QFINDER_CONNECTOR_LANG_PATH . "/" . $_GET['langCode'] . ".php"))
                $langCode = $_GET['langCode'];
        }
        include QFINDER_CONNECTOR_LANG_PATH . "/" . $langCode . ".php";
        if ($number) {
            if (!empty ($GLOBALS['QFLang']['Errors'][$number])) {
                $errorMessage = str_replace("%1", $arg, $GLOBALS['QFLang']['Errors'][$number]);
            } else {
                $errorMessage = str_replace("%1", $number, $GLOBALS['QFLang']['ErrorUnknown']);
            }
        } else {
            $errorMessage = "";
        }
